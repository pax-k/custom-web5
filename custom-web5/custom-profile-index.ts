...
export class ProfileIndex {
  level: MemoryLevel<string, string>;

  constructor(private location = 'data/agent/profiles-index') {
    this.level = new MemoryLevel();
  }
    ...
    for (let propertyName in filter) {
      ...
      if (typeof propertyValue === 'object' && propertyValue !== null) {
        if (Array.isArray(propertyValue)) {
          ...
          const promise = this.buildRangeQuery(
            propertyName,
            propertyValue,
            matches
          );
        ...
        const promise = this.buildTermQuery(
          propertyName,
          propertyValue,
          matches
        );
  ...
  async buildTermQuery(
    propertyName: string,
    propertyValue: unknown,
    matches: { [docId: string]: number }
  ): Promise<void> {
  ...
  async buildRangeQuery(
    ...
    for (let inequality in range) {
      iteratorOptions[inequality] = `${propertyName}~${
        range[inequality as keyof RangeFilter]
      }`;
    }
